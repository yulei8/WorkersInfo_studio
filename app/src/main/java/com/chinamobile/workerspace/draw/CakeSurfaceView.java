...
public class CakeSurfaceView extends SurfaceView implements
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			Log.v("ts", "lastDownX lastDownX:" + lastDownX + ",lastDownY:" + lastDownY);
		...
		case MotionEvent.ACTION_UP:
			...
			float deltaY = Math.abs(lastDownY - firstDownY);
			 Log.v("ts", "changeing xy:" + deltaX + "," + deltaY);
			if (deltaX < 10 && deltaY < 10 && null != itemFrame) {

				int clickPosition = getClickPosition();
				Log.v("ts", "clickPosition:" + clickPosition );
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int measuredHeight = dip2px(getContext(), 1200);
		int measuredWidth = dip2px(getContext(), 840);
		// width模式
		...
		Log.d("OnMeasure", "width:"+widthSpecSize+"height:"+heightSpecSize);
		float cakeSize;
		...
		if (textGravity == Gravity.bottom) {
			...
			if (diffSize > 0) {
				...
				Log.d("OnMeasure", "1 cakeSize:"+cakeSize+"detailHeight:"+detailHeight);
			} else {
				...
				Log.d("OnMeasure", "2 cakeSize:"+cakeSize+"detailHeight:"+detailHeight);
	...
	private void initValueAnimator() {
		...
		rotaValueAnimator.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				startAngle = rota;
				drawCake();
				Log.v("ts", "drawCake startAngle" + startAngle);
